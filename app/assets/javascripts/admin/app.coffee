...
app.go = (url, params) ->
	...
	if app.menu.current
		cur = app.menu.find "[href='/admin/#{app.menu.current}']"
		delete app.menu.current
	else
...
@menu =
	model: (model, name, icon = 'icon-stack') ->
		"<div class='item'>
			...
			<a class='name' href='/admin/model/#{model}' onclick='ripple(event, this); app.aclick(this)'><i class='#{icon}'></i><span>#{name}</span></a>
		</div>"
	...
	html: (role) ->
		app.menu.html "<div class='item'><a href='/' onclick='ripple(event, this)' class='name'><i class='icon-home4'></i>На главную</a></div>#{menu[role]().join ''}<div class='item'><a rel='nofollow' onclick='ripple(event, this)' class='name' data-method='delete' href='/admin/logout'><i class='icon-exit'></i>Выход</a></div>"
ready = ->
	app.yield = $ '#main'
	if !app.menu
		app.menu = $ '#menu'
